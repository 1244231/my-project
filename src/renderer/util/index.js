...
/**
 * 判断类型是否为数组 如果为tru返回原数组 如果为false 返回空数组
 * @param {参数} o 
 * @returns 
 */
export function arrayemptyOrDefault(o) {
    return Object.prototype.toString.call(o) === '[object Array]' ? o : [];

}

/**
 *  定义一个函数，接受一个对象数组和一个字段名作为参数
 *  判断指定属性的值是否有唯一性，相等则返回true不相等返回false
 * @param {*} array 
 * @param {*} field 
 * @returns 
 */
export function hasDifferentValues(array, field) {
    // 如果数组为空或只有一个元素，直接返回false
    if (array.length <= 1) {
        return false;
    }
    // 取出数组中第一个元素的字段值作为参照
    let firstValue = array[0][field];
    // 遍历数组中剩余的元素，如果发现有不同的字段值，返回true
    for (let i = 1; i < array.length; i++) {
        if (array[i][field] !== firstValue) {
            return true;
        }
    }
    // 如果遍历完毕没有发现不同的字段值，返回false
    return false;
}
 ...
 * @returns 
 */
export function isEqualObj(objA, objB) {
    //相等
    ...
    switch (Object.prototype.toString.call(objA)) {
        ...
        case '[object Array]':
            ...
            for (let i = 0; i < objA.length; i++) {
                if (!isEqualObj(objA[i], objB[i])) return false;
            }
            return true;
        case '[object Object]':
            //判断对象
            ...
            for (let i = 0; i < keys.length; i++) {
                if (!isEqualObj(objA[keys[i]], objB[keys[i]])) return false;
            }
